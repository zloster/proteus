...
public class Tests {
	...
	public void exchangeJsonSerialize(HttpServerExchange exchange) {
		try {
			response(objectMapper.writeValueAsString(ImmutableMap.of("message", "Hello, World!")))
					.applicationJson().send(exchange);
		} catch (Exception e) {
	...
	public CompletableFuture<ServerResponse<Map<String, String>>> responseFutureMap(
	...
	public ServerResponse<ByteBuffer> responseUploadFilePath(ServerRequest request,
			@FormParam("file") java.nio.file.Path file) throws Exception {
	...
	public ServerResponse<ByteBuffer> responseUploadOptionalFilePath(ServerRequest request,
			@FormParam("file") Optional<java.nio.file.Path> file) throws Exception {
		if (file.isPresent()) {
			return response(ByteBuffer.wrap(Files.toByteArray(file.get().toFile())))
	...
	public ServerResponse<User> responseEchoJson(ServerRequest request, @FormParam("user") User user)
			throws Exception {
		return response(user).applicationJson();
	}
	...
	public ServerResponse<User> responseInnerClassTest(ServerRequest request, @BeanParam User user)
			throws Exception {
		return response(user).applicationJson();
	}
	...
	public ServerResponse<Set<Long>> genericSet(ServerRequest request,
			@QueryParam("ids") Set<Long> ids) throws Exception {
	...
	public ServerResponse<Set<Long>> genericBeanSet(ServerRequest request, @BeanParam Set<Long> ids)
			throws Exception {
		return response(ids).applicationJson();
	}
	...
	public ServerResponse<List<Long>> genericBeanList(ServerRequest request,
			@BeanParam List<Long> ids) throws Exception {
		return response(ids).applicationJson();
	}
	...
	public ServerResponse<Set<Long>> genericOptionalSet(ServerRequest request,
			@QueryParam("ids") Optional<Set<Long>> ids) throws Exception {
	...
	public ServerResponse<List<Long>> listConversion(ServerRequest request,
			@BeanParam List<Long> ids) throws Exception {
	...
	public ServerResponse<ByteBuffer> timestampConversion(ServerRequest request,
			@QueryParam("timestamp") Timestamp timestamp) throws Exception {
	...
	public ServerResponse<ByteBuffer> instantConversion(ServerRequest request,
			@QueryParam("instant") Instant instant) throws Exception {
	...
	@Consumes("*/*")
	@Operation(description = "Upload file path endpoint")
	public ServerResponse<ByteBuffer> responseUploadByteBuffer(ServerRequest request,
			@FormParam("file") ByteBuffer file) throws Exception {
	...
	@Consumes("*/*")
	@Operation(description = "Upload file path endpoint")
	public ServerResponse<ByteBuffer> responseUploadFile(ServerRequest request,
	...
	@GET
	@Path("response/parameters/path/{pathLong}")
	@Operation(description = "@PathParam parameter" )
	@Produces((MediaType.APPLICATION_JSON))
	public ServerResponse<Map<String,Object>> responsePathParam(
            ServerRequest serverRequest, 
            @PathParam("pathLong")   String pathLongOtherName
            )
    {

		Map<String,Object> responseMap = new HashMap<>();

		responseMap.put("pathLong", pathLongOtherName);

		return response(responseMap).applicationJson(); 
    }
	
	@GET
	@Path("response/parameters/pathregex/{pathLong:[0-9]}")
	@Operation(description = "@PathParam with simple regular expression" )
	@Produces((MediaType.APPLICATION_JSON))
	public ServerResponse<Map<String,Object>> responsePathParamRegEx(
            ServerRequest serverRequest, 
            @PathParam("pathLong")   String pathLongOtherName
            )
    {
